...
# from old_HNOSeq import HNOSimpleSeq as OldHNOSimpleSeq
import datetime
today_date=datetime.datetime.now().strftime('April%d_%H-%M')
device = 'cuda' if torch.cuda.is_available() else torch.device('cpu')
import os

model_date= 'March26_00-46' #'March24_13-45'
...
class ConvDenoiser(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv4(x) #F.relu(self.conv4(x))
        # x = nn.BatchNorm2d(x.size(1)).to(device)(x)
        # x = F.dropout(x, p=0.15, training=self.training)
...
diffusion_steps = 400 ## 700
beta_start = 1e-5 ## 1e-5
beta_end = 0.001 ## 0.001 
schedule_type = 'cosine'  # or 'cosine'
# Choose denoiser_type: 'mlp' or 'conv'
denoiser_type = 'conv'  # or 'conv' 
num_epochs = 50  ## 90
lr=1e-4
normalize=False
...
backbone_loader = DataLoader(backbone_dataset, batch_size=batch_size, shuffle=True)
...
optimizer_backbone = optim.AdamW(model_backbone.parameters(), lr=lr)
...
sidechain_loader = DataLoader(sidechain_dataset, batch_size=batch_size, shuffle=True)
...
optimizer_sidechain = optim.AdamW(model_sidechain.parameters(), lr=lr)
...
pooled_backbone_atoms = samples_backbone[:8] #backbone         #torch.load('./Gens_diffusion//mlpgenerated_sidechainSSeq_March11_2.pt').to(device)
pooled_sidechain_atoms = samples_sidechain[:8]     #gen_sidechain #samples_sidechain  #sidechain     #torch.load('./Gens_diffusion/mlpgenerated_backboneSSeq_March11_2.pt').to(device)

print("pooled_backbone_atoms:")
# print(pooled_backbone_atoms[0])

hidden=checkpoint['hidden']
K=checkpoint['K'] 
window_size=checkpoint['window_size']
num_layers=checkpoint['num_layers']
mlp_layers=checkpoint['mlp']

# model loading
# model_inference = OldHNOSimpleSeq(hidden,K,window_size,num_layers).to(device)
model_inference = HNOSimpleSeq(hidden,K,window_size,num_layers,mlp_layers).to(device)
...
gen_file='generated_result_'+model_date+'.npy'
gen_file='generated_result_groundTruth'+model_date+'.npy'
...
run_folder = f"run_{today_date}"+"diff_"+str(diffusion_steps)+"_beta_"+str(beta_start)+"_"+str(beta_end)+"_"+"norm_"+str(normalize)
# e.g., run_2021-02-01_12-30-45

# Create the directory
os.makedirs('./Recs_Diffusion/PDB/April/'+model_date+'/'+str(run_folder), exist_ok=True)

# Paths to reference PDB and base directory
pdb_file  = f"../heavy_chain.pdb"  # Adjust path
output_directory = './Recs_Diffusion/PDB/April/'+model_date+'/'+str(run_folder)
...
backbone = torch.tensor(backbone).to(device)
sidechain = torch.tensor(sidechain).to(device)
samples_backbone=backbone.reshape(-1,12,130)
samples_sidechain=sidechain.reshape(-1,12,130)


gen_backbone= torch.load('./Gens_diffusion/generated_backbone_March17_last.pt')
gen_sidechain = torch.load('./Gens_diffusion/generated_sidechain_March17_last2.pt')

gen_backbone=torch.tensor(gen_backbone).to(device)
gen_sidechain=torch.tensor(gen_sidechain).to(device)

gen_backbone=gen_backbone.reshape(-1,gen_backbone.shape[2],gen_backbone.shape[3])
gen_sidechain=gen_sidechain.reshape(-1,gen_sidechain.shape[2],gen_sidechain.shape[3])

print(gen_backbone.shape)
print(gen_sidechain.shape)
"""