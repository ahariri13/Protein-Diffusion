...
backbone= np.load('./Poolings/Backbone/train_pooled_backbone_atoms_March02_10-14.npy')
sidechain= np.load('./Poolings/SideChain/train_pooled_sidechain_atoms_March02_10-14.npy')

backbone = torch.tensor(backbone).to(device)
sidechain = torch.tensor(sidechain).to(device)
### Set today date with day hour minur 
import datetime
today_date=datetime.datetime.now().strftime('March%d-%H-%M')
...
torch.save(samples_backbone, 'generated_backboneSimpleSeq_'+str(today_date)+'_'+str(denoiser_type)+'.pt')
torch.save(samples_sidechain, 'generated_sidechainSimpleSeq_'+str(today_date)+'_'+str(denoiser_type)+'.pt')

##############
### Generate proteins and their pdbs 
###############
...
checkpoint_name="./model_March02_10-14.ckpt"
dataRef=dataRef.to(device)

pooled_sidechain_atoms = torch.load('./generated_sidechainSimpleSeq_'+str(today_date)+'_'+str(denoiser_type)+'.pt').to(device)
pooled_backbone_atoms = torch.load('./generated_backboneSimpleSeq_'+str(today_date)+'_'+str(denoiser_type)+'.pt').to(device)
...
### Save results as npy 
import numpy as np
np.save('./generated_result2.npy',result.cpu().numpy())
# torch.save(result, './generated_result.pt')
...
import os
import datetime
npy_file ='./generated_result2.npy'


# Generate a unique folder name based on the current date and time
run_folder = f"run_{today_date}_{diffusion_steps}diff_{beta_end}betaEnd_{num_epochs}epochs" # e.g., run_2021-02-01_12-30-45
