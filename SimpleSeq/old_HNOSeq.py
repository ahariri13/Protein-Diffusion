...
class HNOSimpleSeq(torch.nn.Module):
    ...
    def __init__(self,hiddens_dims,K,window_size,num_layers):
        ...
        self.mlpRep2 = MLP(hiddens_dims, 3, nlayer=2, with_final_activation=True)

        self.convRef = ChebConv(3, hiddens_dims,K=K)
        self.convRef2 = ChebConv(hiddens_dims, hiddens_dims,K=K)
        self.convRef3 = ChebConv(hiddens_dims, hiddens_dims,K=K)
    ...
    def forward(self, x,edge_index,batch,mask,dataRef):
        ...
        for conv in self.convs:
            x_ref = conv(x_ref, edgeref)
            x_ref = nn.BatchNorm1d(x_ref.size(1)).to(device)(x_ref)
            x_ref = F.leaky_relu(x_ref)
            x_ref = F.dropout(x_ref, p=0.1, training=self.training)
